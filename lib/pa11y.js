...
async function setPage(url, options, state) {
	...
	options.log.debug('Checking for Authentication');
	options.log.debug(JSON.stringify(options));

	if (options.username && options.password) {
		options.log.debug('Setting authentication options');
		await state.page.authenticate({'username':options.username, 'password': options.password});
	}	
