...
async function pa11y(url, options = {}, callback) {
	...
	//url = ['https://www.entenmanns.com/en','https://www.entenmanns.com/en/whats-new'];
	...
	try {
		...
		if (options.scanSitemap){
			let sitemapInfo = await option.getURLsFromSitemap(url);
		...
		options.log.info("finally");
		await stateCleanup(options, state);
	}
	// Run callback if present, and resolve with pa11yResults
	options.log.info("pa11yResults");
	options.log.info(pa11yResults);
	return callback ? callback(pa11yError, pa11yResults) : pa11yResults;
}

/**
 * Process Multiple URLs for Runner
 * @private
 * @param {String}||{Array} urls - The URL string or array to run tests against.
 * @param {Object} options - Options to change the way tests run.
 * @returns {Promise} Returns a promise which resolves with a results object.
 */
async function runPa11yTest(urls, options, state) {
	...
	if(Array.isArray(urls)){
		options.log.info(`Multiple URLs found`);

		// Loop through the URL array and display all the values - NL
		for(var url of urls){

		    // let URLresults = await runPa11yTestPage(url, options, state);

		    let URLresults = await promiseTimeout(
						runPa11yTestPage(url, options, state),
						options.timeout,
						`Pa11y timed out (${options.timeout}ms)`
					);

		    //add the URL to each issue
		    URLresults.issues = URLresults.issues.map(v => ({...v, url: url}));

		    if ('issues' in results){
		    	results.issues = results.issues.concat(URLresults.issues);
		    } else {
		    	results = URLresults;
		    }
		...
		options.log.info(`Single URL found.`);
 ...
 * @returns {Promise} Returns a promise which resolves with a results object.
 */
async function runPa11yTestPage(url, options, state) {
	//The current pa11y internal state, fields will be mutated by this function.
